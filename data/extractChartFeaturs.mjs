...
specs.forEach(async file=>  {
    console.log('--------------', file, '--------------');
    if (fs.statSync(`${folder}/${file}`).isDirectory()){// ignore directory
        console.log('this is folder');
        return;
    }
    
    //vega-lite to vega spec
    let liteSpec = fs.readFileSync(`${folder}/${file}`);
    liteSpec = JSON.parse(liteSpec.toString())
    const spec = compile(liteSpec).spec; 
    
    // fix data urls
    spec.data.forEach((d) => {
        if (d.url && d.url.startsWith("data")) {
            const filename = d.url.split('/').pop();
            d.url = `${folder}/data/${filename}`;
        }
    });
    // feature extraction and export
    const features = await extractFeatures(spec);
    fs.writeFileSync(`${output_folder}/${file}`, JSON.stringify(features))
    
    
});

// Copied from src/utils/extractChartFeatures.js
async function extractFeatures(spec) {
