...
export default memo(function ChartBlock({
  ...
  const links = useSelector(state => Object.values(state.links).filter(link => link.chartId === blockProps.id), shallowEqual);
  const spec = useMemo(() => addSignalToChartSpec(JSON.parse(JSON.stringify(blockProps.spec))), [blockProps.spec]);
  ...
  useEffect(() => {
    ...
    if (links.some(link => link.active)){
      ...
      links.filter(link => link.active).forEach(link=>{
        view.signal('highlight', { data:[link.text], field: link.feature.field, enabled:true }).run();
      })
    }else{
  ...
  return (
    ...
    <div ref={containerEl} {...elementProps} style={{ ...style }}
