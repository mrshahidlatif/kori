...
export default function Editor(props) {
  ...
  const existingLinks = useSelector((state) => state.links);
  const charts = useSelector((state) => getCharts(state, docId));
  const chartsInEditor = useSelector((state) =>
    getChartsInEditor(state, docId)
  );
  ...
  function linkAlreadyExists(existingLinks, text) {
    let exists = false;
    Object.keys(existingLinks).forEach(function (key) {
      exists = existingLinks[key].fullText === text;
    });
    return exists;
  }
  function findAndInsertLinks(chartsInEditor, editorState, text, type) {
    const links = findLinks(chartsInEditor, text, type);
  ...
  function handleEditorChange(editorState) {
    ...
    let exists = false;
    exists = linkAlreadyExists(existingLinks, lastTypedWord.text);
    if (!exists && lastTypedWord !== null) {
      editorState = findAndInsertLinks(
        chartsInEditor,
        editorState,
        lastTypedWord,
        "WORD"
    ...
    if (lastSentence !== null)
      exists = linkAlreadyExists(existingLinks, lastSentence.text);
    if (!exists && lastSentence !== null) {
      editorState = findAndInsertLinks(
        chartsInEditor,
        editorState,
        lastSentence,
