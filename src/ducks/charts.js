...
export const getCharts = (state) => {
  ...
  return state.ui.currentDocId
    ? state.docs[state.ui.currentDocId].charts.map((cid) => state.charts[cid])
    : [];
};
...
export const getChartsInEditor = (state) => {
  //TODO: memoization using reselect
  return state.ui.currentDocId
    ? state.docs[state.ui.currentDocId].chartsInEditor.map(
        (cid) => state.charts[cid]
      )
    : [];
};
// reducers
